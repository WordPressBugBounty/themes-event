					...
					<div class="container">
						...
						<?php if($event_get_booking_posts->have_posts()):$event_get_booking_posts->the_post();
							if($i==1 &&  $event_settings['event_title'] !=''){
								...
								<li><i class="fa-regular fa-calendar-days"></i><?php echo esc_html($event_booking_title_attribute); ?></li>
								<?php $i++;
							}
						endif;
							if($event_settings['event_date_time'] !=''){
								echo '<li>'.esc_html($event_settings['event_date_picker']).'&nbsp;&nbsp;&nbsp;'.esc_html($event_settings['event_date_time']).'</li>';
							}
							if($event_settings['event_venue'] !=''){
								echo '<li>'.esc_html($event_settings['event_venue']).'</li>';
							} ?>
						</ul>
						<?php if($event_settings['event_book_appointment'] !=''){ ?>
						<div class="alignright">
						<?php if($event_settings['event_book_appointment_url']!=''){ ?>
							<a class="appointment-btn btn-eff" href="<?php echo esc_url($event_settings['event_book_appointment_url']); ?>"><i class="fa-solid fa-pen-to-square"></i><?php echo esc_html($event_settings['event_book_appointment']); ?></a>
						<?php }else{ ?>
							<a class="appointment-btn btn-eff" href="<?php the_permalink(); ?>"><i class="fa-solid fa-pen-to-square"></i><?php echo esc_html($event_settings['event_book_appointment']); ?></a>
						<?php } ?>
						</div>
						<?php  } ?>
					</div>	
