	...
	if($query->have_posts() && get_option( 'sticky_posts' )){
			...
			$image_attributes = wp_get_attachment_image_src($attachment_id,'event_slider_image');
				...
				if ($title_attribute != '' || $excerpt != '') {
					...
					$event_sticky_post_sliders_display 	.= '<h4 class="slider-sub-title">'.esc_html($cat_name).'</h4><!-- .slider-title -->';
				$remove_link = $event_settings['event_slider_link'];
					...
					if ($excerpt != '') {
						...
						if(!empty($slider_custom_text)){
							$event_sticky_post_sliders_display 	.= '<a title="'.esc_attr($slider_custom_text).'"' .' href="'.esc_url($slider_custom_url). '"'. ' class="btn-default vivid" target="_blank">'.esc_html($slider_custom_text). '</a>';
						}
						if($event_settings['event_slider_button'] == 0){
							if($excerpt_text == '' || $excerpt_text == 'Read More') :
								...
								$event_sticky_post_sliders_display 	.= '<a title='.'"'.get_the_title(). '"'. ' '.'href="'.esc_url(get_permalink()).'"'.' class="btn-default light">'.esc_html($event_settings[ 'event_tag_text' ]).'</a>';
							endif;
						}
...
function event_scripts() {
	...
	/********* Adding Fonts ********************/
	wp_register_style( 'event_google_fonts', wptt_get_webfont_url('https://fonts.googleapis.com/css?family=Roboto+Condensed:400,400italic,700,300' ));
